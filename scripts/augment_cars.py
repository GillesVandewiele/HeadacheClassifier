...
descriptors = [(DISCRETE, len(np.unique(df['buying']))), (DISCRETE, len(np.unique(df['maint']))),
               (DISCRETE, len(np.unique(df['doors']))), (DISCRETE, len(np.unique(df['persons']))),
               (DISCRETE, len(np.unique(df['lug_boot']))), (CONTINUOUS,),
               (DISCRETE, len(np.unique(df['safety']))), (CONTINUOUS,)]
...
fig = plt.figure()
fig.suptitle('Impact of data augmentation using the cars dataset', fontsize=15)
counter = 0
for tree in trees:
