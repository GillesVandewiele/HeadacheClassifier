...
for train_index, test_index in skf:
    ...
    for tree_constructor in tree_constructors:
        ...
        region = merger.decision_tree_to_decision_table(tree, features_df)
        ...
        region_predicted_labels = merger.evaluate_regions(region, test_features_df)
        tree_confusion_matrices[tree_constructor.get_name()+" Tree"].append(tree.plot_confusion_matrix(test_labels_df['cat']
                                                                            .values.astype(str), predicted_labels.astype(str)))

        tree_confusion_matrices[tree_constructor.get_name()+" Regions"].append(DecisionTree.plot_confusion_matrix(test_labels_df['cat']
                                                                            .values.astype(str), region_predicted_labels.astype(str)))



    best_tree, best_regions = merger.genetic_algorithm(train_df, 'cat', tree_constructors, seed=SEED, num_iterations=10,
                                         num_mutations=5, population_size=10, max_samples=8, val_fraction=0.2) #7 maxsam
    ...
    tree_confusion_matrices["Genetic Tree"].append(DecisionTree.plot_confusion_matrix(test_labels_df['cat'].values.astype(str),
                                              predicted_labels.astype(str)))
    predicted_labels = merger.evaluate_regions(best_regions, test_features_df)
    tree_confusion_matrices["Genetic Regions"].append(DecisionTree.plot_confusion_matrix(test_labels_df['cat'].values.astype(str),
                                              predicted_labels.astype(str)))
...
for key in tree_confusion_matrices:
    ...
    ax = fig.add_subplot(2, len(tree_confusion_matrices)/2, counter+1)
    cax = ax.matshow(cm_normalized, cmap=plt.cm.Blues, vmin=0.0, vmax=1.0)
    ax.set_title(key, y=1.04)
